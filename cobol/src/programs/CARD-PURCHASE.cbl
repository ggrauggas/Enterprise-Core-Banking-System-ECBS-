      ******************************************************************
      * CARD-PURCHASE.cbl - Fase 7: card purchase (compra).
      *
      * Bridge input (one JSON line on stdin):
      *   {"cardId":1,"amount":250.75,
      *    "description":"Supermarket","user":"pos-terminal"}
      *
      * Rules: card must exist (E004) and be ACTIVE (E008 if
      * BLOCKED, E020 if CANCELLED), amount > 0 (E018) and the
      * available credit must cover it (E009 CREDIT LIMIT EXCEEDED).
      * Atomically: available_credit decrease + CARD_PURCHASE row
      * in transactions (on the linked account) + audit entry.
      ******************************************************************
       IDENTIFICATION DIVISION.
       PROGRAM-ID. CARD-PURCHASE.
      ...
      * framework module parameter blocks
       ...
       MAIN-PARA.
           ...
           IF WS-OK = 'Y' PERFORM APPLY-PURCHASE END-IF
       ...
       FETCH-CARD.
           ...
           EVALUATE TRUE
               ...
               WHEN WS-CARD-STATUS = 'CANCELLED'
                   ...
                   PERFORM SET-CARD-CTX
                   MOVE 'N' TO WS-OK
               WHEN WS-CREDIT-OK NOT = 1
                   MOVE 'E009' TO WS-ERR-CODE
           ...
           EXEC SQL
               INSERT INTO transactions
                      (account_id, transaction_type, amount,
                       description)
               VALUES (:WS-ACCT-ID, 'CARD_PURCHASE', :WS-AMOUNT,
       ...
       PRINT-OK.
           ...
           MOVE 'CARD-PURCH' TO LOG-COMPONENT
           MOVE SPACES TO LOG-MSG
           MOVE WS-CARD-ID TO WS-ID-ED
           MOVE WS-TX-ID TO WS-TX-ED
           STRING 'purchase txn=' FUNCTION TRIM(WS-TX-ED)
