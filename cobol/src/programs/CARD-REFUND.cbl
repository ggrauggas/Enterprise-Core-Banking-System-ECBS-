      ******************************************************************
      * CARD-REFUND.cbl - Fase 7: card refund (devolucion).
      *
      * Bridge input (one JSON line on stdin):
      *   {"cardId":2,"amount":100.00,
      *    "description":"Returned goods","user":"pos-terminal"}
      *
      * Rules: card must exist (E004) and be ACTIVE (E008/E020),
      * amount > 0 (E018) and the refund must not push the
      * available credit above the credit limit (E021) - you can
      * only get back what was spent. Atomically: available_credit
      * increase + CARD_REFUND row in transactions + audit entry.
      ******************************************************************
       IDENTIFICATION DIVISION.
       PROGRAM-ID. CARD-REFUND.
      ...
      * SQL host variables (declared inline so ocesql can see them)
       ...
       01  WS-REFUND-OK            PIC 9.
       01  WS-OLD-AVAIL            PIC X(20).
       01  WS-LIMIT-TXT            PIC X(20).
       01  WS-NEW-AVAIL            PIC X(20).
      ...
      * framework module parameter blocks
       ...
       MAIN-PARA.
           ...
           IF WS-OK = 'Y' PERFORM APPLY-REFUND END-IF
       ...
       FETCH-CARD.
           EXEC SQL
               SELECT status, account_id,
                      ...
                      CASE WHEN available_credit + :WS-AMOUNT
                                <= credit_limit
                           THEN 1 ELSE 0 END
                 INTO :WS-CARD-STATUS, :WS-ACCT-ID,
                      :WS-OLD-AVAIL, :WS-LIMIT-TXT, :WS-REFUND-OK
                 FROM cards
           ...
           EVALUATE TRUE
               ...
               WHEN WS-CARD-STATUS = 'CANCELLED'
                   ...
                   PERFORM SET-CARD-CTX
                   MOVE 'N' TO WS-OK
               WHEN WS-REFUND-OK NOT = 1
                   MOVE 'E021' TO WS-ERR-CODE
                   MOVE SPACES TO WS-ERR-CTX
                   STRING 'AVAILABLE '
                          FUNCTION TRIM(WS-OLD-AVAIL)
                          ' LIMIT '
                          FUNCTION TRIM(WS-LIMIT-TXT)
       ...
       APPLY-REFUND.
           ...
           END-IF
           EXEC SQL
               SELECT CAST(available_credit AS VARCHAR),
                      CAST(:WS-AMOUNT AS VARCHAR)
                 INTO :WS-NEW-AVAIL, :WS-AMT-TXT
                 FROM cards
                WHERE card_id = :WS-CARD-ID
           END-EXEC
           IF SQLCODE NOT = 0
           ...
           EXEC SQL
               INSERT INTO transactions
                      (account_id, transaction_type, amount,
                       description)
               VALUES (:WS-ACCT-ID, 'CARD_REFUND', :WS-AMOUNT,
       ...
       PRINT-OK.
           ...
           MOVE 'CARD-REFUND' TO LOG-COMPONENT
           MOVE SPACES TO LOG-MSG
           MOVE WS-CARD-ID TO WS-ID-ED
           MOVE WS-TX-ID TO WS-TX-ED
           STRING 'refund txn=' FUNCTION TRIM(WS-TX-ED)
                  ' card=' FUNCTION TRIM(WS-ID-ED)
                  ' amount=' FUNCTION TRIM(WS-AMT-TXT)
