      ******************************************************************
      * CARD-UNBLOCK.cbl - Fase 7: card unblocking (desbloqueo).
      *
      * Bridge input (one JSON line on stdin):
      *   {"cardId":1,"user":"operator1"}
      *
      * Only a BLOCKED card can be unblocked: ACTIVE -> E014,
      * CANCELLED -> E020. The status change is audited.
      ******************************************************************
       IDENTIFICATION DIVISION.
       PROGRAM-ID. CARD-UNBLOCK.
      ...
      * framework module parameter blocks
       ...
       MAIN-PARA.
           ...
           IF WS-OK = 'Y' PERFORM UNBLOCK-CARD END-IF
       ...
       FETCH-CARD.
           ...
           EVALUATE TRUE
               ...
               WHEN WS-CARD-STATUS = 'CANCELLED'
                   ...
                   PERFORM SET-CARD-CTX
                   MOVE 'N' TO WS-OK
               WHEN WS-CARD-STATUS NOT = 'BLOCKED'
                   MOVE 'E014' TO WS-ERR-CODE
                   MOVE 'CARD NOT BLOCKED' TO WS-ERR-CTX
                   MOVE 'N' TO WS-OK
           END-EVALUATE.
       ...
       WRITE-AUDIT.
           ...
           STRING '{"status":"ACTIVE","cardNumber":"'
                  WS-CARD-NUM '"}'
       ...
       PRINT-OK.
           ...
           MOVE 'CARD-UNBLOCK' TO LOG-COMPONENT
           MOVE SPACES TO LOG-MSG
           MOVE WS-CARD-ID TO WS-ID-ED
           STRING 'card unblocked id=' FUNCTION TRIM(WS-ID-ED)
