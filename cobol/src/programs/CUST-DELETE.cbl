      ******************************************************************
      * CUST-DELETE.cbl - Fase 4: logical customer deletion (baja).
      *
      * Bridge input (one JSON line on stdin):
      *   {"customerId":5,"user":"operator1"}
      *
      * The row is never physically removed: status is moved to
      * 'DELETED' so history (accounts, transactions, audit) stays
      * intact. Deleting a missing or already deleted customer is
      * rejected. The status change is written to audit_logs.
      ******************************************************************
       IDENTIFICATION DIVISION.
       PROGRAM-ID. CUST-DELETE.
      ...
      * framework module parameter blocks
       ...
       MAIN-PARA.
           ...
           IF WS-OK = 'Y' PERFORM DELETE-CUSTOMER END-IF
       ...
       FETCH-CURRENT.
           EXEC SQL
               SELECT status, email
                 INTO :WS-OLD-STATUS, :WS-OLD-EMAIL
                 FROM customers
           ...
           EVALUATE TRUE
               ...
               WHEN WS-OLD-STATUS = 'DELETED'
                   ...
                   MOVE 'CUSTOMER ALREADY DELETED' TO WS-ERR-CTX
                   MOVE 'N' TO WS-OK
           END-EVALUATE.
       ...
       WRITE-AUDIT.
           ...
           STRING '{"status":"' FUNCTION TRIM(WS-OLD-STATUS)
                  '","email":"' FUNCTION TRIM(WS-OLD-EMAIL) '"}'
               DELIMITED BY SIZE INTO WS-AUD-OLD
           END-STRING
           MOVE SPACES TO WS-AUD-NEW
           STRING '{"status":"DELETED","email":"'
                  FUNCTION TRIM(WS-OLD-EMAIL) '"}'
       ...
       PRINT-OK.
           ...
           MOVE 'CUST-DELETE' TO LOG-COMPONENT
           MOVE SPACES TO LOG-MSG
           MOVE WS-CUST-ID TO WS-ID-ED
           STRING 'customer logically deleted id='
