      ******************************************************************
      * TXN-WITHDRAW.cbl - Fase 6: withdrawal (valida fondos).
      *
      * Bridge input (one JSON line on stdin):
      *   {"accountId":1,"amount":200.00,
      *    "description":"ATM withdrawal","user":"operator1"}
      *
      * Rules: account must exist and be ACTIVE, amount > 0 and
      * balance >= amount (E005 INSUFFICIENT FUNDS otherwise; the
      * database CHECK (balance >= 0) is the last line of defense).
      * Atomically: balance decrease + transactions row
      * (WITHDRAWAL) + audit_logs entry.
      ******************************************************************
       IDENTIFICATION DIVISION.
       PROGRAM-ID. TXN-WITHDRAW.
      ...
      * framework module parameter blocks
       ...
       MAIN-PARA.
           ...
           IF WS-OK = 'Y' PERFORM APPLY-WITHDRAWAL END-IF
           ...
           EXEC SQL
               SELECT status, CAST(balance AS VARCHAR),
                      CASE WHEN balance >= :WS-AMOUNT
                           THEN 1 ELSE 0 END
                 INTO :WS-ACCT-STATUS, :WS-OLD-BAL, :WS-FUNDS-OK
                 FROM accounts
           ...
           EXEC SQL
               INSERT INTO transactions
                      (account_id, transaction_type, amount,
                       description)
               VALUES (:WS-ACCT-ID, 'WITHDRAWAL', :WS-AMOUNT,
       ...
       PRINT-OK.
           ...
           MOVE 'TXN-WITHDRAW' TO LOG-COMPONENT
           MOVE SPACES TO LOG-MSG
           MOVE WS-ACCT-ID TO WS-ID-ED
           MOVE WS-TX-ID TO WS-TX-ED
           STRING 'withdrawal txn=' FUNCTION TRIM(WS-TX-ED)
